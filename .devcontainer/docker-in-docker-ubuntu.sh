...
if [ "${USERNAME}" = "auto" ] || [ "${USERNAME}" = "automatic" ]; then
    USERNAME=""
    POSSIBLE_USERS=("intellij" "codespace" "$(awk -v val=1000 -F ":" '$3==val{print $1}' /etc/passwd)")
...
ARCH=$(uname -m)
curl -SL "https://github.com/docker/compose/releases/download/v${COMPOSE_VERSION}/docker-compose-linux-${ARCH}" -o /usr/local/bin/docker-compose
